...
function initPixiTexture() {
  ...
  const baseTexture = new PIXI.BaseTexture(
    new PIXI.resources.BufferResource(textureArr, {
      width: WIDTH,
      height: HEIGHT,
    })
  );
...
function clearAnts() {
  for (const ant of ants) {
    ...
    if (ground === 3) {
      GROUND[y][x] = 0;
      fermoneD[y][x] =
        fermoneD[y][x] > fermoneIntensity ? fermoneD[y][x] : fermoneIntensity;
    }
    ...
    if (ground === 2 && ant.find === "food") {
      ...
      ant.fermoneD = FOODF;
      ant.fermoneF = HOMEF;
      ant.fermoneIntensity +=
        ant.fermoneIntensity <= 3 ? ANT_FERMONE_STRENGTH : 0;
      ant.angle += Math.PI;
    }
...
const updateAnts = gpu.createKernel(
  function (ants, FOODF, HOMEF, GROUND) {
